...
class OLED(Extension):
    ...
    def after_matrix_scan(self, sandbox):
        if sandbox.active_layers[0] != self.prev_active_layer:
            self.draw_layer_name(self.oleds[0], sandbox.active_layers)
            self.draw_badge(
                self.oleds[0],
                self.icon.mode_badges[sandbox.active_layers[0]][1],
            )
            self.prev_active_layer = sandbox.active_layers[0]
        #print(sandbox.matrix_update)
        return
    ...
    def draw_layer_name(self, oled, active_layers):
        ...
        mid = int(
            len(self.layer_names[active_layers[0]]) / 2
            + len(self.layer_names[active_layers[0]]) % 2
        )

        # draw current mode name
        oled.text(
            self.layer_names[active_layers[0]], center - (mid * 5), 0, 1
        )
        oled.show()
        # set prev mode name
        self.prev_layer_name = self.layer_names[active_layers[0]]
